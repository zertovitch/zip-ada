...
procedure Entropy_Segmentation is
  ...
  procedure Segment (seg : out Index_Vectors.Vector) is
    ...
    use REF;
    discrepancy_threshold : constant := 1.9;      --  Discrepancy detection threshold.
    index_threshold       : constant := 20_000;   --  Do segmentation only above a certain distance.
    window_size           : constant := 10_000;   --  Sliding window size.
...
begin
  ...
  Create (f_csv, Out_File, "entropy_" & Simple_Name (Name (f_in)) & ".csv");
