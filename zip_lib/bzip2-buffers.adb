package body BZip2.Buffers is
  ...
  procedure Flush_Bit_Buffer (bit_buffer : in out Bit_Buffer_Type) is
    use Interfaces;
  begin
    bit_buffer.destination.pos := bit_buffer.destination.pos + 1;
    bit_buffer.destination.data (bit_buffer.destination.pos) := bit_buffer.buffer;
    bit_buffer.buffer := 0;
    bit_buffer.pos    := 7;
  ...
  begin
    for count in reverse 1 .. amount loop
      if (data and Shift_Left (Unsigned_32'(1), count - 1)) /= 0 then
        bit_buffer.buffer := bit_buffer.buffer or Shift_Left (Unsigned_8'(1), bit_buffer.pos);
      end if;
      if bit_buffer.pos = 0 then
        Flush_Bit_Buffer (bit_buffer);
      else
        bit_buffer.pos := bit_buffer.pos - 1;
      end if;
    end loop;
  end Put_Bits;
